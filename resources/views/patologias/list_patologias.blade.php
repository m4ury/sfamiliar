            ...
            @foreach($paciente->patologias as $patologia)
                <ul class="list-group">
                    ...
                    @if($patologia->nombre_patologia == 'HTA' or $patologia->nombre_patologia == 'DM2')
                        {{-- RAC vigente--}}
                        {{--dd(Carbon\Carbon::parse($paciente->racVigente)->diffInDays(Carbon\Carbon::now())) <= 365--}}
                            <div class="form-group row">
                                @if(\Carbon\Carbon::parse($paciente->racVigente)->diffInDays(\Carbon\Carbon::now()) <= 365)
                                    <div class="col sm-3">
                                        ...
                                        <br>
                                        <p class="btn rounded-pill bg-gradient-success">fecha: {{ $paciente->racVigente }}</P>
                                    </div>
                        ...
                        @if($patologia->nombre_patologia == 'DM2')
                            ...
                            @if(Carbon\Carbon::parse($paciente->vfgVigente)->diffInDays(Carbon\Carbon::now()) < 365)
                                    <div class="col-sm-3">
                                        ...
                                        <p class="btn rounded-pill bg-gradient-success">fecha:{{ $paciente->vfgVigente }}</P>
                                    </div>
                                ...
                                {{--fondo de ojo--}}
                                    @if(Carbon\Carbon::parse($paciente->fondoOjoVigente)->diffInDays(\Carbon\Carbon::now()) < 365)
                                        <div class="col-sm-4">
                                            ...
                                            <p class="btn rounded-pill bg-gradient-success">fecha: {{ $paciente->fondoOjoVigente }}</P>
                                        </div>
                                    ...
                                    @if(Carbon\Carbon::parse($paciente->ecgVigente)->diffInDays(\Carbon\Carbon::now()) < 365)
                                        <div class="col-sm-3">
                                            ...
                                            <p class="btn rounded-pill bg-gradient-success">fecha: {{ $paciente->ecgVigente }}</P>
                                        </div>
